...
get_record = """
    SELECT *, EXTRACT(EPOCH FROM(current_timestamp at time zone 'UTC')-date_recorded)/3600 AS record_age
    FROM {0}
...
delete_over_3hours = """
    DELETE FROM {0}
    WHERE EXTRACT(EPOCH FROM(current_timestamp at time zone 'UTC')-date_recorded)/3600 > 3;
"""
...
try:
    ...
    # Clear > 90 days
    cur.execute(delete_over_3hours.format('msp_data_capture'))
    cur.execute(delete_over_3hours.format('msp_data_capture_level2'))
    cur.execute(delete_over_3hours.format('msp_data_capture_level3'))
    conn.commit()
